/*
* 创建人 张南宁
* 和之前引用pagination分页插件不同 配置项更多更灵活
*/
/**
 * pagination分页插件
 * @version 1.3.1
 ...
 * 
 * 组件样式与功能分离 如需自定义样式 只需覆盖pagination.css 
 * 或者更改函数_addStyle里的CSS_PATH路径为自定义css文件路径
 *
 * @调用方法
 * $(selector).pagination();
 */
	...
	var Pagination = function(element,options){
		...
		*/
		this.setTotalData = function(totalData){
			...
			opts.current = (parseInt(opts.totalData / opts.showData) + 1) > this.getCurrent() ? this.getCurrent() :　0;
			this.init();
		};
		 ...
		 * 获取总条数
		*/
		this.getTotalData = function(totalData){
		...
		this.filling = function(index){
			var html = '';
			current = Number(index) || opts.current;//当前页码
	...
	*/
    ...
    function _getCssPath(){
    	$(document.scripts).each(function(index, el) {
			if(el.src.indexOf("jquery.pagination.js") > -1){
				BASE_PATH = el.src;
				CSS_PATH = el.src.replace("jquery.pagination.js","pagination.css");
			}
		});
    }
